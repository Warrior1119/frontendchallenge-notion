...
const Label: FC<Props> = forwardRef((props, ref) => {
  const { children, title, position, disabled, tabIndex, onKeyPress, className, ...otherProps } = props;
  ...
  return (
    <label className={classes} ref={ref} tabIndex={tabIndex} onKeyPress={onKeyPress} role='presentation' {...otherProps}>
      {text}
      {children}
    </label>
  );
